...
from .tts import build_client, list_voices, synthesize
...
def _common_options(f):
    ...
    f = click.option("-o", "--output", "output_path", type=click.Path(path_type=Path), default=None,
                      help="Save the generated audio to this path (default: a temp cache file).")(f)
...
def _run_tts(raw_text: str, *, api_key, voice_id, model_id, output_path, play, raw, max_chars) -> None:
    ...
    dest = output_path or (Path.home() / ".cache" / "vocalize" / "last.mp3")
