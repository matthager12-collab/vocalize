...
check 'run-14-local-llm validated' grep -q '^validate-exit: PASS' docs/plans/2026-09-app-roadmap/run-14-local-llm/report.md
check "run 14's key artifact exists" test -f vocalize/local/llm_worker.py
...
echo ""
echo "=== Exit criteria ==="
check 'pipeline and negatives' .venv/bin/python -m pytest tests/test_notes.py -q -p no:cacheprovider
check 'segments contract' .venv/bin/python -m pytest tests/test_whisper_worker.py -q -p no:cacheprovider -k segments
check 'threads clean' .venv/bin/python -m pytest tests/test_notes.py -q -p no:cacheprovider -k threads
