#!/usr/bin/env bash
# validate-exit.sh — Run 7: hotkey-spike-and-builder
#
...
PREV_REPORT="docs/plans/2026-09-app-roadmap/run-6-release-0-12-0/report.md"

echo "=== Entry criteria ==="
check "on its own branch, not main" bash -c 'test "$(git branch --show-current)" != "main"'
check "run 6 (0.12.0 release) validated" bash -c "grep -q '^validate-exit: PASS' '$PREV_REPORT'"
check "0.12.0 wheel exists" bash -c 'ls dist/vocalize_cli-0.12.0-*.whl >/dev/null 2>&1'
