#!/usr/bin/env bash
# validate-exit.sh — Run 14: local-llm
#
...
check "run 13 (spikes) validated" grep -q '^validate-exit: PASS' docs/plans/2026-09-app-roadmap/run-13-spikes/report.md
check "T-121 done: spike-notes.md § LLM fully answered" bash -c '
  f=docs/plans/2026-09-app-roadmap/spike-notes.md
  test -f "$f" &&
  [[ "$(grep -A12 "^## LLM" "$f" | grep -cE "^(signatures|offline|model_type|template|shards|cold|warm):")" -eq 7 ]]
'
...
echo ""
echo "=== Exit criteria ==="
check "manifest hardened (test_llm_manifest.py)" .venv/bin/python -m pytest tests/test_llm_manifest.py -q -p no:cacheprovider
check "worker discipline (test_llm_worker.py)" .venv/bin/python -m pytest tests/test_llm_worker.py -q -p no:cacheprovider
check "install lifecycle (cli/local_install/readiness -k llm)" .venv/bin/python -m pytest tests/test_cli.py tests/test_local_install.py tests/test_readiness.py -q -k llm -p no:cacheprovider
check "local backend uses --offline (test_llm.py -k local)" .venv/bin/python -m pytest tests/test_llm.py -q -k local -p no:cacheprovider
check "no llm runtime leaked into the wheel" bash -c '
  set -e
  rm -rf dist /tmp/vocalize-run14-clean-venv
  .venv/bin/python -m build -o dist >/dev/null
  python3 -m venv /tmp/vocalize-run14-clean-venv
  /tmp/vocalize-run14-clean-venv/bin/pip install -q --no-cache-dir dist/vocalize_cli-*.whl
  ! /tmp/vocalize-run14-clean-venv/bin/pip list | grep -iE "pywhispercpp|onnxruntime|mlx|sherpa|numpy|torch|boto3"
'
