...
check "package builds the 0.12.0 wheel" bash -c 'rm -f dist/vocalize_cli-0.12.0*; .venv/bin/python -m build >/dev/null 2>&1; ls dist/vocalize_cli-0.12.0*.whl >/dev/null 2>&1'
check "clean-venv install has no leaked ML runtime" bash -c '
  ...
  /tmp/vocalize-0-12-0-cleanvenv/bin/pip install -q --no-cache-dir dist/vocalize_cli-0.12.0*.whl
