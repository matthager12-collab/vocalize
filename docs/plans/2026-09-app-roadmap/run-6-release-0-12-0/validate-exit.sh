#!/usr/bin/env bash
# validate-exit.sh — Run 6: release-0-12-0
#
...
PASS=0
FAIL=0
# The full suite ran 213s on this machine at entry (736+ tests) — 300s leaves
# headroom over the 120s default so "full suite green" isn't a coin-flip timeout.
...
check "run 5 (keys tab) validated" grep -q '^validate-exit: PASS' docs/plans/2026-09-app-roadmap/run-5-keys-tab/report.md
check "run 5's key artifact present (auth.KEY_SLOTS)" grep -q "KEY_SLOTS" vocalize/auth.py
...
echo ""
echo "=== Exit criteria ==="
check "review findings file exists" test -f docs/plans/2026-09-app-roadmap/review-0.12.0.md
check "no open critical/high finding" .venv/bin/python -c 'import re,pathlib; t=pathlib.Path("docs/plans/2026-09-app-roadmap/review-0.12.0.md").read_text(); assert not re.search(r"^\| *(critical|high) *\|.*\| *open *\|", t, re.I|re.M), "open critical/high finding"'
check "CHANGELOG has 0.12.0" grep -qE '^## .*0\.12\.0' CHANGELOG.md
check "version bumped to 0.12.0" grep -qE '^__version__ = "0\.12\.0"' vocalize/__init__.py
check "docs match the CLI (0.12.0 commands)" .venv/bin/python -c 'import subprocess; [subprocess.run([".venv/bin/vocalize", *c.split(), "--help"], check=True, capture_output=True) for c in ("listen", "dictate", "resume", "status", "auth login")]'
...
check "package builds the 0.12.0 wheel" bash -c 'rm -rf /tmp/vocalize-0-12-0-build; .venv/bin/python -m build --outdir /tmp/vocalize-0-12-0-build >/dev/null 2>&1; ls /tmp/vocalize-0-12-0-build/vocalize_cli-0.12.0*.whl >/dev/null 2>&1'  # a scratch dir: dist/ holds the published files the digest row compares
check "clean-venv install has no leaked ML runtime" bash -c '
  set -e
  rm -rf /tmp/vocalize-0-12-0-cleanvenv
  python3 -m venv /tmp/vocalize-0-12-0-cleanvenv
  /tmp/vocalize-0-12-0-cleanvenv/bin/pip install -q --no-cache-dir /tmp/vocalize-0-12-0-build/vocalize_cli-0.12.0*.whl
  ! /tmp/vocalize-0-12-0-cleanvenv/bin/pip list | grep -iE "pywhispercpp|onnxruntime|mlx|sherpa|numpy|torch|boto3"
'
check "PyPI 0.12.0 published with matching digest (after the owner publishes)" .venv/bin/python -c 'import json,urllib.request,hashlib,glob,sys; local={hashlib.sha256(open(f,"rb").read()).hexdigest() for f in glob.glob("dist/vocalize_cli-0.12.0*")}; data=json.load(urllib.request.urlopen("https://pypi.org/pypi/vocalize-cli/0.12.0/json", timeout=20)); remote={u["digests"]["sha256"] for u in data["urls"]}; assert local and local==remote, (local, remote)'
