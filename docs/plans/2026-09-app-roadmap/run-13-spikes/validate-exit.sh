#!/usr/bin/env bash
# validate-exit.sh — Run 13: spikes
#
...
PREV_REPORT="docs/plans/2026-09-app-roadmap/run-12-release-0-13-1/report.md"
...
check "run 12 (0.13.1 release) validated" bash -c "grep -q '^validate-exit: PASS' '$PREV_REPORT'"
check "0.13.1 wheel exists" bash -c 'ls dist/vocalize_cli-0.13.1-*.whl >/dev/null 2>&1'
...
if grep -A25 '^### DEC-034' docs/plans/2026-09-app-roadmap/decisions.md | grep -q 'Decision.*A'; then
  check "Parakeet engine tests green (go only)" .venv/bin/python -m pytest tests/ -q -k parakeet -p no:cacheprovider
else
  check "Parakeet no-go recorded (DEC-034 B)" bash -c "grep -A25 '^### DEC-034' docs/plans/2026-09-app-roadmap/decisions.md | grep -q 'Decision.*B'"
fi
