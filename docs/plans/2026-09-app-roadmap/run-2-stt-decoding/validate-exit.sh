#!/usr/bin/env bash
# validate-exit.sh — Run 2: stt-decoding
#
...
echo "=== Entry criteria ==="
check "on its own branch (not main)" test "$(git branch --show-current)" != main
check "run 1 (local-first defaults) validated" grep -q '^validate-exit: PASS' docs/plans/2026-09-app-roadmap/run-1-local-first-defaults/report.md
check "run 1 artifact: default chain flipped" grep -q 'DEFAULT_CHAIN = ("kokoro", "say")' vocalize/config.py
...
echo ""
echo "=== Exit criteria ==="
check "beam search on, with an escape hatch (tests)" .venv/bin/python -m pytest tests/test_whisper_worker.py tests/test_config.py -q -k beam -p no:cacheprovider
check "beam cost measured: spike-notes.md holds 4 timings" bash -c 'test -f docs/plans/2026-09-app-roadmap/spike-notes.md && [ "$(grep -A6 "^## Beam" docs/plans/2026-09-app-roadmap/spike-notes.md | grep -cE "^(cold|warm) beam [15]:")" = "4" ]'
check "turbo q8_0 row pinned in the manifest (test)" .venv/bin/python -m pytest tests/test_whisper_manifest.py -q -k q8_0 -p no:cacheprovider
check "turbo q8_0 documented in docs/dictation.md" grep -c q8_0 docs/dictation.md
check "CoreML measured: spike-notes.md holds 8 figures + a verdict" bash -c 'test -f docs/plans/2026-09-app-roadmap/spike-notes.md && [ "$(grep -A10 "^## CoreML" docs/plans/2026-09-app-roadmap/spike-notes.md | grep -cE "^(cold|warm|rss)")" = "8" ] && grep -A10 "^## CoreML" docs/plans/2026-09-app-roadmap/spike-notes.md | grep -q "^verdict:"'
