#!/usr/bin/env bash
# validate-exit.sh — Run 7: portal-read
#
...
echo ""
echo "=== Exit criteria ==="
check 'portal module exists' test -f vocalize/portal.py
check 'auth invariants incl. Host on every route and lockout' .venv/bin/python -m pytest tests/test_portal.py -k 'token or host or session or lockout' -q -p no:cacheprovider
check 'security headers on every response' .venv/bin/python -m pytest tests/test_portal.py -k headers -q -p no:cacheprovider
check 'state route returns under a blocked probe' .venv/bin/python -m pytest tests/test_portal.py -k state -q -p no:cacheprovider
check 'CSP string is the designed one' grep -qE 'media-src '"'"'self'"'"' blob:' vocalize/portal.py
