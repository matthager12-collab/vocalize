#!/usr/bin/env bash
# validate-exit.sh — Run 10: release-0-11-0
#
...
echo ""
echo "=== Exit criteria ==="
check 'review findings file exists' test -f docs/plans/2026-09-next-features/review-0.11.0.md
check 'no open critical/high finding' .venv/bin/python -c 'import re,pathlib; t=pathlib.Path('"'"'docs/plans/2026-09-next-features/review-0.11.0.md'"'"').read_text(); assert not re.search(r'"'"'^\| *(critical|high) *\|.*\| *open *\|'"'"', t, re.I|re.M), '"'"'open critical/high finding'"'"''
check 'CHANGELOG has 0.11.0' grep -qE '^## .*0\.11\.0' CHANGELOG.md
check 'version bumped to 0.11.0' grep -qE '^__version__ = "0\.11\.0"' vocalize/__init__.py
check 'README documents the portal' grep -qE 'vocalize portal' README.md
